...
import { Edit3, Trash2, PlusCircle, Search, Eye, PackagePlus, Filter, Grid, List, TrendingUp, ChevronDown } from 'lucide-react';
...
const AdminViewProductsByCategory = () => {
  const [selectedCategory, setSelectedCategory] = useState(AVAILABLE_CATEGORIES[0]?.value || '');
  ...
  const fetchProductsByCategory = async (category) => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_URL}/api/products/${encodeURIComponent(category)}`);
      
      if (!response.ok) {
        let errorDetails = `Erreur HTTP ${response.status} lors de la récupération des produits pour la catégorie ${category}.`;
        try {
      ...
      console.error(`Erreur lors de la récupération des produits pour la catégorie ${category}:`, err);
      setError(err.message || `Impossible de charger les produits pour la catégorie ${category}.`);
      setProducts([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredProducts = products.filter(product => {
    ...
    return productName.includes(lowerSearchTerm) || productIdString.includes(lowerSearchTerm);
  });
  ...
  const currentCategoryLabel = AVAILABLE_CATEGORIES.find(c => c.value === selectedCategory)?.label || selectedCategory;
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div>
                ...
                <p className="text-gray-600 mt-2 text-lg">
                  Explorez vos produits par catégorie
                </p>
              </div>
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-black text-white px-6 py-4 pr-12 rounded-xl font-semibold shadow-lg hover:from-indigo-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all duration-200 cursor-pointer text-lg min-w-64"
                >
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-600 mb-6">
                {searchTerm 
                  ? `Aucun produit ne correspond à "${searchTerm}" dans ${currentCategoryLabel}.`
                  : `Aucun produit dans la catégorie ${currentCategoryLabel}.`}
              </p>
              {!searchTerm && (
                <Link
            ...
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 ">
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white w-full rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                    <div className="aspect-square bg-gray-100 relative overflow-hidden">
                      <img 
                        src={`${API_URL}${product.path || '/assets/default-product.jpg'}`}
                        alt={product.name || 'Image produit'} 
                        className="w-full h-full object-cover transition-transform duration-300 hover:scale-105" 
                        onError={(e) => {e.target.src = `${API_URL}/assets/default-product.jpg`}}
                      />
                    ...
                    <div className="p-5">
                      <div className="mb-3">
                        <p className="text-sm font-medium text-indigo-600 mb-1">{currentCategoryLabel}</p>
              ...
              <divddd className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
...
export default AdminViewProductsByCategory;