...
function Germain() {
  ...
  const handleCatSelect = (cat) => {
    ...
    if (cat === "Tri par prix croissante") {
      setProducts([...tab.english].sort((a, b) => parseInt(a.price) - parseInt(b.price)));
    } else if (cat === "Tri par prix decroissante") {
      setProducts([...tab.english].sort((a, b) => parseInt(b.price) - parseInt(a.price)));
    } else {
      setProducts(tab.english);
    }
  };

  useEffect(() => {
    setProducts(tab.english.filter(item => parseInt(item.price) >= minPrice && parseInt(item.price) <= maxPrice));
  }, [minPrice, maxPrice]);
