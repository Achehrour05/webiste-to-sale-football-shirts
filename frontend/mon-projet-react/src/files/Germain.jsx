...
function Germain() {
  ...
  const handleCatSelect = (cat) => {
    ...
    if (cat === "Tri par prix croissante") {
      setProducts([...tab.germain].sort((a, b) => parseInt(a.price) - parseInt(b.price)));
    } else if (cat === "Tri par prix decroissante") {
      setProducts([...tab.germain].sort((a, b) => parseInt(b.price) - parseInt(a.price)));
    } else {
      setProducts(tab.germain);
    }
  };

  useEffect(() => {
    setProducts(tab.germain.filter(item => parseInt(item.price) >= minPrice && parseInt(item.price) <= maxPrice));
  }, [minPrice, maxPrice]);
