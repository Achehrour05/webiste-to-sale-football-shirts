...
function Italie() {
  ...
  const [products, setProducts] = useState(tab.italie);  // Use the shared product list for Italie
  ...
  const handleCatSelect = (cat) => {
    ...
    if (cat === "Tri par prix croissante") {
      setProducts([...tab.italie].sort((a, b) => parseInt(a.price) - parseInt(b.price)));
    } else if (cat === "Tri par prix decroissante") {
      setProducts([...tab.italie].sort((a, b) => parseInt(b.price) - parseInt(a.price)));
    } else {
      setProducts(tab.italie);
    }
  };

  useEffect(() => {
    setProducts(tab.italie.filter(item => parseInt(item.price) >= minPrice && parseInt(item.price) <= maxPrice));
  }, [minPrice, maxPrice]);
