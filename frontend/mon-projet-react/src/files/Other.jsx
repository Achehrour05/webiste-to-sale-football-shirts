...
function Other() {
  ...
  const [products, setProducts] = useState(tab.other);  // Use the shared product list for Other
  ...
  const handleCatSelect = (cat) => {
    ...
    if (cat === "Tri par prix croissante") {
      setProducts([...tab.other].sort((a, b) => parseInt(a.price) - parseInt(b.price)));
    } else if (cat === "Tri par prix decroissante") {
      setProducts([...tab.other].sort((a, b) => parseInt(b.price) - parseInt(a.price)));
    } else {
      setProducts(tab.other);
    }
  };

  useEffect(() => {
    setProducts(tab.other.filter(item => parseInt(item.price) >= minPrice && parseInt(item.price) <= maxPrice));
  }, [minPrice, maxPrice]);
