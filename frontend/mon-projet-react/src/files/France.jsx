...
function France() {
  ...
  const [products, setProducts] = useState(tab.france);  // Use the shared product list for France


  useEffect(() => {
    setProducts(tab.france.filter(item => parseInt(item.price) >= minPrice && parseInt(item.price) <= maxPrice));
  }, [minPrice, maxPrice]);
